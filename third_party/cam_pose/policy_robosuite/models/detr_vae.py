...
class DETRVAE(nn.Module):
    def __init__(self, backbone, transformer, encoder, action_dim, obs_dim, chunk_size):
        ...
        self.input_proj_cam_extrinsics = nn.Linear(16, hidden_dim)
        # Shared positional embedding for [extr0, extr1, proprio, latent]
        self.input_embed = nn.Embedding(4, hidden_dim)
    ...
    def forward(self, data):
        ...
        cam_extrinsics = data.get('cam_extrinsics')  # (B, N, 4, 4)
        ...
        extr_tokens = self.input_proj_cam_extrinsics(cam_extrinsics.reshape(bs, 2, 16))
        ...
        src = torch.cat([camera_features, extr_tokens, proprio_input, latent_input], dim=1) 
        proprio_latent_pos_embed = repeat(self.input_embed.weight, 's d -> b s d', b=bs)  # (bs, 4, hidden_dim)
